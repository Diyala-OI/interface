 ...
 */
	...
	function connect() {
		...
		exit('<h2>'.$this->_setError().' Please notify <a href="mailto:diyala@irkalla.net">administrator</a></h2>');
		return false;
		}
 ...
 */
	function fetchRow($sql = NULL) {
		if ($this->_currentRow >= $this->_numRows) {
			if (oci_statement_type($this->_statementId) != 0){
...
echo '<pre>';print_r($q);echo '</pre>';
				...
				return $this->_mergeHasMany($resultSet, $fetch, $association, $Model, $Model, $recursive);
...
$count = count($query['joins']);
		...
		$this->renderStatement('select', array(
